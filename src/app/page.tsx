            ...
            <div className="md:hidden">
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="ghost" size="icon" className="text-primary hover:bg-primary/10"><Menu className="h-6 w-6" /></Button>
                </SheetTrigger>
                <SheetContent side="right" className="w-[300px]">
                  ...
                  <nav className="flex flex-col gap-1 mt-6">
                    <SheetClose asChild><Link href="/programs/dashboard" className="text-base font-semibold py-3 px-3 rounded-lg hover:bg-secondary/50">Programs</Link></SheetClose>
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div className="space-y-8 max-w-2xl">
                <h1 className="text-5xl lg:text-7xl font-headline font-bold leading-[1.1] text-primary">Master Your Future with <span className="text-accent underline decoration-accent/30 underline-offset-8">RP Coach-Up</span></h1>
                <p className="text-xl text-muted-foreground leading-relaxed">The smarter way to find tuition. We connect students with the perfect courses and teachers based on shared interests and goals.</p>
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  <Button size="lg" className="h-14 px-8 text-lg rounded-xl" disabled={isLoggedIn} asChild={!isLoggedIn}>{isLoggedIn ? <span>Find a Tutor</span> : <Link href="/register?role=student">Find a Tutor</Link>}</Button>
                  <Button size="lg" variant="outline" className="h-14 px-8 text-lg rounded-xl border-2" disabled={isLoggedIn} asChild={!isLoggedIn}>{isLoggedIn ? <span>Join as Teacher</span> : <Link href="/register?role=teacher">Join as Teacher</Link>}</Button>
                </div>
              </div>
              <div className="relative group">
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-border aspect-[16/9]">
                  <Image src={heroImg?.imageUrl || "/images/hero-education.jpg"} alt="Students learning" fill className="object-cover" priority />
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="features" className="py-24 bg-background">
          <div className="container mx-auto px-4">
            <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
              ...
              <p className="text-lg text-muted-foreground">Our platform streamlines the search for quality education. We facilitate meaningful connections to ensure the best learning outcomes.</p>
            </div>
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="space-y-6">
                <div className="grid gap-8">
                  {[
                    { icon: Search, title: "Smart Search", description: "Easily find courses and teachers that align with your unique interests and learning styles." },
                    { icon: Users, title: "Collaborative Learning", description: "Connect with teachers who are passionate about the exact topics you want to master." },
                    { icon: Zap, title: "Fast Results", description: "Submit your interests and get personalized connection suggestions in a streamlined portal." }
                  ].map((feature, i) => (
                    <div key={i} className="flex gap-4">
                      <div className="bg-primary/10 p-3 rounded-xl h-fit"><feature.icon className="h-6 w-6 text-primary" /></div>
                      <div><h3 className="text-xl font-headline font-bold mb-1">{feature.title}</h3><p className="text-muted-foreground">{feature.description}</p></div>
                    </div>
                  ))}
                </div>
              </div>
              <div className="relative rounded-2xl overflow-hidden border shadow-xl bg-muted aspect-[4/5]">
                 <Image src={mentorImg?.imageUrl || "/images/teacher-mentoring.jpg"} alt="Educational mentoring" fill className="object-cover" />
              </div>
            </div>
          </div>
        </section>
